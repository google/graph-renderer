 ...
 */
export const BUILT_IN_MARKER_DEFINITIONS: Readonly<Record<Exclude<EndpointMarker, EndpointMarker.NONE>, Omit<CustomEndpointMarker, 'id'|'color'>>> = {
...
function createMarkerDefs(
  ...
  for (const [color, markers] of markerMap.entries()) {
    for (const markerType of markers) {
      ...
      const markerDef = BUILT_IN_MARKER_DEFINITIONS[markerType as Exclude<EndpointMarker, EndpointMarker.NONE>];
      if (markerDef) {
        defs.push({
          id,
          color,
...
export class EdgeCanvas extends LitElement {
  ...
  >([]);
  private readonly customEndpointMarkers$ = new BehaviorSubject<CustomEndpointMarker[]>([]);
  ...
  override connectedCallback() {
    ...
    const markerDefs$ = combineLatest([this.edges$, this.customEndpointMarkers$]).pipe(
