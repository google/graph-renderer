...
describe('Minimap', () => {
  const state = cleanState(async () => {
    // Mocking Pointer Capture APIs (unsupported in JSDOM)
  ...
  it('should calculate derived state and styles correctly', async () => {
    ...
    // Using Regex to allow for JSDOM's 0 vs 0px unit normalization
    expect(state.viewBox.style.transform).toMatch(
      /translate3d\(0px, 0px, 0(px)?\)/
    );
  ...
  it('should dispatch a pan event on click', () => {
    ...
    // Access call arguments via mock.calls[index][argIndex]
    const eventDetail = state.panSpy.mock.calls[0][0].detail;
  ...
  it('should dispatch pan events when dragging the viewbox', () => {
    ...
    expect(state.panSpy).toHaveBeenCalledTimes(1);
    let eventDetail = state.panSpy.mock.calls[0][0].detail;
    ...
    eventDetail = state.panSpy.mock.calls[1][0].detail;
    ...
    // Get the third call (index 2)
    eventDetail = state.panSpy.mock.calls[2][0].detail;
