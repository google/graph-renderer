...
describe('GraphRenderer Utils', () => {
  ...
  describe('isPoint', () => {
    it('returns false for an endpoint', () => {
      ...
      expect(utils.isPoint(endpoint)).toBeFalse();
    });
    ...
    it('returns false for a node', () => {
      ...
      expect(utils.isPoint(node)).toBeFalse();
    });

    it('returns true for a point', () => {
      const point = {x: 1, y: 2};
      expect(utils.isPoint(point)).toBeTrue();
    });
  });
  ...
  describe('isWheelEventOverScrollable', () => {
    ...
    it('should return true for an event over a scrollable element', () => {
      ...
      const event = new WheelEvent('wheel', {bubbles: true, composed: true, deltaY: 10});
      ...
      expect(utils.isWheelEventOverScrollable(event, wrapper)).toBeTrue();
    });
    ...
    it('should return false for an event over a non-scrollable element', () => {
      ...
      expect(utils.isWheelEventOverScrollable(event, wrapper)).toBeFalse();
    });
    ...
    it('should return false if the search stops before the scrollable element', () => {
      ...
      expect(utils.isWheelEventOverScrollable(event, scrollableChild)).toBeFalse();
    });

    it('should return false if there is no scroll delta', () => {
      // deltaY is 0, so no scroll is happening.
      const event = new WheelEvent('wheel', {bubbles: true, composed: true, deltaY: 0});
      Object.defineProperty(event, 'composedPath', {
        value: () => [innerContent, scrollableChild, wrapper],
      });
