...
describe('GraphRenderer', () => {
  const state = cleanState(async () => {
    ...
    const graphPanSpy = vi.fn();
    const graphZoomSpy = vi.fn();
    ...
    const resizeObserverObserveSpy = vi.spyOn(
      ResizeObserver.prototype,
      'observe'
    );
    const resizeObserverDisconnectSpy = vi.spyOn(
      ResizeObserver.prototype,
      'disconnect'
    );
  ...
  describe('Panning', () => {
    ...
    it('emits graph-pan events on pointer drag on wrapper', async () => {
      ...
      expect(state.element.isPanning).toBe(false);
      ...
      expect(state.element.isPanning).toBe(true);
      expect(state.graphPanSpy).toHaveBeenCalledTimes(2);
      expect(state.graphPanSpy.mock.calls[0][0].detail.event.type).toBe(
        'start'
      );
      expect(state.graphPanSpy.mock.calls[1][0].detail.event.type).toBe('move');
      ...
      expect(state.graphPanSpy.mock.calls[2][0].detail.event.type).toBe('move');
      ...
      expect(state.graphPanSpy.mock.calls[3][0].detail.event.type).toBe('end');
  ...
  describe('Resize Observation', () => {
    it('should use observeResizeElement for ResizeObserver if provided', async () => {
      ...
      expect(state.resizeObserverObserveSpy).toHaveBeenCalledWith(
        externalElement
      );
