...
[![npm version](https://img.shields.io/npm/v/@google/graph-renderer.svg)](https://www.npmjs.com/package/@google/graph-renderer)

**[View the Interactive Demo](https://google.github.io/graph-renderer/index.html)**
...
## Project Context
Developed to provide a standardized, high-performance graph visualization component for Google Cloud and AI platforms. It is currently the primary renderer for:
*   **[Google Gemini Enterprise](https://cloud.google.com/gemini-enterprise)**: Graph-based reasoning and data relationship views.
*   **[Google Cloud Application Integration](https://cloud.google.com/application-integration)**: Visual designer to orchestrate enterprise workflows.
...
-   [Development and Scripts](#development-and-scripts)
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/google/graph-renderer.git
    cd graph-renderer
    ```
...
This project is built with **Vite** for fast development and **Vitest** for unit testing. It adheres to **Google TypeScript Style (GTS)** for linting and formatting.

### Demo Deployment
The interactive demo is automatically deployed to GitHub Pages via GitHub Actions on every push to the `main` branch.
**Live Demo:** [https://google.github.io/graph-renderer/index.html](https://google.github.io/graph-renderer/index.html)
...
| `npm run build` | Compiles the demo site for production (output to `/dist`). |
| `npm run build:lib` | Compiles the library for npm distribution (ES modules and types). |
| `npm run preview` | Serves the production build locally for testing. |
| `npm run test` | Runs Vitest in watch mode for active development. |
| `npm run test:run` | Executes the test suite once (used in CI/CD). |
| `npm run lint` | Checks for style and linting violations using GTS. |
| `npm run fix` | Automatically corrects style issues and formats code. |
| `npm run typecheck` | Validates TypeScript types across the project. |

### Build Modes
- **Demo Mode**: Used for GitHub Pages. Assets are bundled with a base path of `/graph-renderer/`.
- **Library Mode** (`--mode plugin`): Used for npm publishing. Externalizes core dependencies like `lit`, `rxjs`, and `elkjs` to ensure the final bundle is lightweight for consumers.
