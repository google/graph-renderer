...
export class GraphRenderer extends LitElement {
  ...
  override disconnectedCallback() {
    ...
    if (this.wrapper) {
      ...
      this.wrapper.removeEventListener('wheel', this.handleWheelEvent, {capture: true});
      this.wrapper.removeEventListener('wheel', this.handleWheelEvent, {capture: false});
    }
  ...
  private setupResizeObserver() {
    ...
    if (!target) {
      console.error('gr-graph-renderer: No valid element to observe (observeResizeElement or wrapper).');
      return;
    }
    ...
    if (!(target instanceof Element)) {
       console.warn('gr-graph-renderer: Target for ResizeObserver is not a valid Element.', target);
       return;
    }
    ...
    try {
      this.resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          ...
          if (this.viewportDimension.width !== dims.width || this.viewportDimension.height !== dims.height) {
      ...
      console.error('gr-graph-renderer: Failed to observe element', error, target);
    }
  }
  ...
  private readonly handlePointerUp = (event: PointerEvent) => {
    ...
    this.wrapper.addEventListener('click', consumeClick, {once: true, capture: true});
  ...
  private updateWheelListener() {
    ...
    const isCapture = this.mouseWheelBehavior === MouseWheelBehavior.ZOOM_CAPTURES;
  ...
  private handleNodeDragEnd(event: CustomEvent<Point & {id: string}>) {
    ...
    const nextGraphNodes = currentGraphNodes.map((node) => {
      return node.id === detail.id ? {...node, x: detail.x, y: detail.y} : node;
    });
    ...
    return html`
      <div class="wrapper">
        <div class="background-canvas">
          <svg width="100%" height="100%" style="position: absolute; background-color: ${
        this.theme.background.fill};">
            <defs>
              <pattern
                id="${this.backroundId}"
                x="0"
                ...
                <circle
                  ...
                  fill="${this.theme.background.dots.fill}"></circle>
              </pattern>
            </defs>
            <rect
              width="100%"
              height="100%"
              fill="${this.backgroundUrl}"></rect>
          </svg>
        </div>
        <gr-directed-graph
        ...
        ${this.showMinimap ? html`
          <div class="minimap-container">
            <gr-minimap
              ...
              @minimap-pan=${this.handleMinimapPan}></gr-minimap>
          </div>
        ` : ''}
      </div>
    `;
