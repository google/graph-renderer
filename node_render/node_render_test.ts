...
describe('NodeRender', () => {
  const state = cleanState(async () => {
    ...
    const dragStartSpy = jasmine.createSpy('node-drag-start');
    const dragMoveSpy = jasmine.createSpy('node-drag-move');
  ...
  it('does not emit node-drag-start on pointerdown', () => {
    ...
    expect(state.element.isDragging).toBeFalse();
  });
  ...
  it('emits node-drag-start on pointermove after exceeding threshold', () => {
    ...
    expect(state.dragStartSpy.calls.mostRecent().args[0].detail).toEqual(
      BASE_NODE,
    );
    expect(state.element.isDragging).toBeTrue();
  });
  ...
  it('emits node-drag-move on pointermove after drag has started', async () => {
    ...
    let moveEventDetail = state.dragMoveSpy.calls.mostRecent().args[0].detail;
    ...
    moveEventDetail = state.dragMoveSpy.calls.mostRecent().args[0].detail;
    expect(moveEventDetail).toEqual({
  ...
  it('emits node-drag-end on pointerup and resets dragging state immediately', async () => {
    ...
    const endEventDetail = state.dragEndSpy.calls.mostRecent().args[0].detail;
  ...
  it('constrains drag within bounds if constrainNodeDrag is true', async () => {
    ...
    const moveEventDetail = state.dragMoveSpy.calls.mostRecent().args[0].detail;
