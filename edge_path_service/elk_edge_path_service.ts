...
export class ElkEdgePathService extends EdgePathService {
  ...
  constructor(config?: ElkEdgePathConfig) {
    ...
    this.config = {
      ...
      originSplitRadius: config?.originSplitRadius ?? DEFAULT_ORIGIN_SPLIT_RADIUS,
      terminalBendRadius: config?.terminalBendRadius ?? DEFAULT_TERMINAL_BEND_RADIUS,
      defaultBendRadius: config?.defaultBendRadius ?? DEFAULT_BEND_RADIUS,
      targetMarkerClearance: config?.targetMarkerClearance ?? DEFAULT_TARGET_MARKER_CLEARANCE,
  ...
  buildPath<T extends BaseNode, M extends BaseEdge | TentativeEdge>(
    ...
    for (let sIdx = 0; sIdx < sections.length; sIdx++) {
      ...
      const points = snappedPoints.slice(currentPointIndex, currentPointIndex + numSectionPoints);
      ...
      const dLast = Math.hypot(targetPoint.x - lastRefPoint.x, targetPoint.y - lastRefPoint.y);
      ...
      if (isLastSection && this.config.targetMarkerClearance > 0) {
        ...
        endPoint = {
          x: targetPoint.x - (shortening * (targetPoint.x - lastRefPoint.x)) / (dLast || 1),
          y: targetPoint.y - (shortening * (targetPoint.y - lastRefPoint.y)) / (dLast || 1),
        };
      }
    ...
    switch (this.config.labelPositioning) {
      ...
      case ElkLabelPositioning.PATH_MIDPOINT:
        ...
        for (let i = 0; i < allPoints.length - 1; i++) {
          totalLength += Math.hypot(allPoints[i + 1].x - allPoints[i].x, allPoints[i + 1].y - allPoints[i].y);
        }
        ...
        if (totalLength > 0) {
          ...
          for (let i = 0; i < allPoints.length - 1; i++) {
            const d = Math.hypot(allPoints[i + 1].x - allPoints[i].x, allPoints[i + 1].y - allPoints[i].y);
            if (traversed + d >= target) {
              const ratio = (target - traversed) / (d || 1);
              labelPosition = {
                x: allPoints[i].x + ratio * (allPoints[i + 1].x - allPoints[i].x),
                y: allPoints[i].y + ratio * (allPoints[i + 1].y - allPoints[i].y),
              };
              break;
            }
            traversed += d;
