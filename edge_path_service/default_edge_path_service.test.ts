  ...
  describe('static methods', () => {
    ...
    describe('#linearInterpolate', () => {
      ...
      it('throws an error if interpolatePercent is > 1', () => {
        ...
        }).toThrow('interpolatePercent must be between 0 and 1');
      });
    ...
    describe('#getMarkerOrientation', () => {
      ...
      describe('when the marker is at the end of the line', () => {
        it.each([96, 150, 204])(
          'returns RIGHT orientation if y is %i',
          pointY => {
            expect(
        ...
        it.each([96, 150, 204])(
          'returns LEFT orientation if y is %i',
          pointY => {
            expect(
              DefaultEdgePathService.getMarkerOrientation(
                { x: 205, y: pointY },
        ...
        it.each([96, 150, 204])(
          'returns DOWN orientation if x is %i',
          pointX => {
            expect(
              DefaultEdgePathService.getMarkerOrientation(
                { x: pointX, y: 95 },
        ...
        it.each([96, 150, 204])('returns UP orientation if x is %i', pointX => {
          expect(
            DefaultEdgePathService.getMarkerOrientation(
              { x: pointX, y: 205 },
              relatedNode,
    ...
    describe('#calculateAngleFromPositiveX', () => {
      it.each([
        { p: { x: 1, y: 0 }, expected: 0 },
        { p: { x: 1, y: 1 }, expected: 45 },
        { p: { x: 0, y: 1 }, expected: 90 },
        { p: { x: -1, y: 1 }, expected: 135 },
        { p: { x: -1, y: 0 }, expected: 180 },
        { p: { x: -1, y: -1 }, expected: 225 },
        { p: { x: 0, y: -1 }, expected: 270 },
        { p: { x: 1, y: -1 }, expected: 315 },
      ])('returns angle $expected for point $p', ({ p, expected }) => {
        expect(
