...
const EDGE_WITH_INVALID_SECTIONS: BaseEdge = {
  ...
  to: {nodeId: TO_NODE.id},
  sections: [
    {id: 's1', startPoint: {x: 10, y: 20}}, // Missing endPoint
    {id: 's2', endPoint: {x: 100, y: 100}}, // Missing startPoint
  ],
};
...
describe('ElkEdgePathService', () => {
  ...
  describe('buildPath', () => {
    ...
    describe('when "to" is a Node', () => {
      ...
      it('builds a path from multiple sections', () => {
        ...
        const expectedEndPointX = section2.endPoint!.x - TARGET_MARKER_CLEARANCE;
    ...
    it('respects the clearance guard for extremely short final segments', () => {
      const clearance = 10;
      const shortService = new ElkEdgePathService({targetMarkerClearance: clearance});
  ...
  describe('snapThreshold features', () => {
    it('snaps points within the threshold to eliminate layout jitter', () => {
      const threshold = 5;
      const snappingService = new ElkEdgePathService({snapThreshold: threshold});
      const startPoint = {x: 0, y: 0};
